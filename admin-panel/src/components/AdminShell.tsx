                ...
                {navItems.map(({ tab, label, icon: Icon }) => (
                  <button key={tab} onClick={() => onNavigate?.(tab)}
                    className={`relative flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition ${
                      activeTab === tab ? "gradient-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                    }`}>
            ...
            {showDeliveryViews && (
              <div className="hidden items-center gap-1 rounded-full border border-border bg-background/60 p-1 sm:flex">
                {([
                  ...
                  <button key={id} onClick={() => onDeliveryTabChange?.(id)}
                    className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition ${
                      activeDeliveryTab === id ? "gradient-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                    }`}>
                    <Icon className="h-3.5 w-3.5" />
                    {label}
                  </button>
                ...
                {profileOpen && (
                  <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="px-3 py-2">
                      ...
                      <div className="mt-1 inline-flex rounded-full bg-primary/10 px-2 py-0.5 text-[10px] uppercase tracking-wide text-primary">{user.role}</div>
                    </div>
            ...
            <motion.div ref={drawerRef}
              ...
              {user && (
                <div className="mx-4 mt-4 rounded-2xl border border-border bg-background p-3">
                  ...
                  <div className="mt-2 inline-flex rounded-full bg-primary/10 px-2.5 py-0.5 text-[10px] uppercase tracking-wide text-primary">{user.role}</div>
                </div>
              )}
              ...
              {showAdminNav && (
                <nav className="mt-4 flex-1 space-y-1 px-4">
                  ...
                  {navItems.map(({ tab, label, icon: Icon }) => (
                    <button key={tab} onClick={() => { onNavigate?.(tab); setDrawerOpen(false); }}
                      className={`relative flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition ${
                        activeTab === tab ? "gradient-primary text-primary-foreground shadow-sm" : "hover:bg-accent text-foreground"
                      }`}>
                      <Icon className="h-4 w-4 shrink-0" />
                      {label}
                      {tab === "bulk-orders" && pendingBulk > 0 && (
                        <span className="ml-auto grid h-5 w-5 place-items-center rounded-full bg-amber-500 text-[10px] font-black text-white">{pendingBulk}</span>
                      )}
                    </button>
                  ))}
                </nav>
                  ...
                  {[
                    ...
                    <button key={id} type="button"
                      ...
                      className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition ${
                        activeDeliveryTab === id ? "gradient-primary text-primary-foreground shadow-sm" : "hover:bg-accent text-foreground"
                      }`}>
                      <Icon className="h-4 w-4 shrink-0" />
                      {label}
                    </button>
