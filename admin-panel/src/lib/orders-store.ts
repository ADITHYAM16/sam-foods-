...
export async function assignNearestAgent(orderId: string) {
  const { data: agents } = await (adminClient.from("delivery_agents") as any)
  ...
  await (adminClient.from("delivery_requests") as any).insert(rows);
}
...
export async function sendDeliveryRequestToNext(orderId: string, excludeAgentIds: string[]) {
  ...
  const counts: { id: string; count: number }[] = await Promise.all(
    eligible.map(async (a) => {
      const { count } = await (adminClient.from("orders") as any)
  ...
  await (adminClient.from("delivery_requests") as any).insert({
    order_id: orderId,
    agent_id: least.id,
    status: "pending",
  });
...
export function useDeliveryOrders(agentId?: string | null): { orders: Order[]; loading: boolean; newAlert: Order | null; clearAlert: () => void } {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const { data: accepted } = await (adminClient.from("delivery_requests") as any)
        .select("order_id")
        .eq("agent_id", agentId)
        .eq("status", "accepted");
  ...
  useEffect(() => {
      ...
      .on("postgres_changes",
        ...
        async ({ new: row }) => {
          ...
          if (r.status === "denied") {
            ...
            const { data: ord } = await (adminClient.from("orders") as any)
    ...
    const ordersChannel = adminClient
    ...
    return () => {
      adminClient.removeChannel(drChannel);
      adminClient.removeChannel(ordersChannel);
    };
  }, [fetchOrders, agentId]);
