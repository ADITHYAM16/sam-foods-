...
import { Eye, EyeOff, Loader2, Mail, Lock, User, Phone, ChefHat } from "lucide-react";
...
function LoginPage() {
  ...
  const reset = () => { setErr(null); setEmail(""); setPassword(""); setName(""); setPhone(""); setConfirm(""); setAgree(false); };
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await register({ name, email: email.trim(), phone, password, role: "customer" });
      navigate({ to: "/" });
    } catch (e) {
